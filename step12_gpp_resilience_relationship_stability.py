...
if __name__ == '__main__':
    ...
    # Ta
    Ta = np.load(current_dir+'/1_Input/data for drivers/tmmx_16d_rm_seasonality.npy')[:,460:460+460]
    Ta_sd = smooth_2d_array(moving_sd(Ta,23).T, 5)
    # plt.figure(); plt.plot(np.nanmean(Ta_sd, axis=0))

    # Srad
    Srad = np.load(current_dir + '/1_Input/data for drivers/srad_16d_rm_seasonality.npy')[:,460:460+460]
    ...
    fig, axs = plt.subplots(1, 2, figsize=(10 * 0.8, 3.5 * 0.8))
    ...
    axs[1].set_ylabel("SD of Ta")
    ax1.set_ylabel("SD of Srad")
    ax2.set_ylabel("SD of Pr")
