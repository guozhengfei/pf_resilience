...
if __name__ == '__main__':
    ...
    pr01 = tf.imread(Data_folder + 'soil_moisture_1982_2022-0000000000-0000000000.tif')
    pr02 = tf.imread(Data_folder + 'soil_moisture_1982_2022-0000000000-0000002304.tif')
    ...
    Evi_sea = np.nanmean(np.reshape(EVI, (EVI.shape[0], int(EVI.shape[1] / 23), bands_year)), axis=1)
    # plt.figure(); plt.plot(np.nanmean(Evi_sea,axis=0))

    Evi_sea_rep = np.zeros_like(EVI)
    for yr in range(yr_num):
        Evi_sea_rep[:, yr * 23:(yr + 1) * 23] = Evi_sea
    res = EVI - Evi_sea_rep
    np.isnan(res).sum() / (res.shape[0] * res.shape[1])
    res[np.isnan(res)] = 0
    ...
    np.save(current_dir+'/1_Input/data for drivers/sm_16d_rm_seasonality.npy', EVI_res)