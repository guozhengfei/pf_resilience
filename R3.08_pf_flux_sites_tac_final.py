...
OUTPUT_DIR = Path('../4_Figures/flux_site_tac')
BANDS_YEAR = 12
TAC_YEARS = 3
...
for site, df_site in df.groupby('Site'):
    ...
    df_site.loc[df_site.index[:12], 'tac'] = np.nan
    df_site.loc[df_site.index[-12:], 'tac'] = np.nan
    if df_site['tac'].mean()<-0.1: df_site['tac'] = df_site['tac'] - np.nanmin(df_site['tac'])
...
df_out.to_csv('/Volumes/Zhengfei_01/Project 2 pf resilience/1_Input/pf_flux_sites_monthly_residual_tac_final.csv', index=False)
