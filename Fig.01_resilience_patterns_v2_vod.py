...
if __name__ == '__main__':
    ...
    tac = ar1_5yr_modis
    tac[tac == 0] = np.nan;
    tac_yr = tac.reshape(tac.shape[0], 22, 23)
    ...
    time_label_modis = np.linspace(2002,2019,18)
    ...
    trend = np.load(current_dir + '/2_Output/spatial_resilience/resilience_trend_vod.npy')
    trend_07 = trend[:, 0] * 23
    ...
    fig.tight_layout()
    figToPath = current_dir+'/4_Figures/Fig01_resilience_patterns_temporal_vod'
    ...
    cdsi_trends_df.to_csv(current_dir + '/4_Figures/Fig.1/Fig01_cdsi_trends_vod.csv', index=False)