...
if __name__ == '__main__':
    ...
    data = np.load(current_dir+ '/2_Output/Temporal/Temporal_r_shap_obs_pre_opt_vod_fast_fitted.npz')
    coefs = data['array1']
    mask = data['array2']
    ...
    mask = ((abs(obs.reshape(-1)-pred.reshape(-1))<0.3)&(obs.reshape(-1)>0.08))
    obs_fit = obs.reshape(-1)[mask]
    pred_fit = pred.reshape(-1)[mask]
    fit = st.linregress(obs_fit, pred_fit)

    fig, axs = plt.subplots(1,2, figsize=(5*1.2, 2.5*1.2))
    axs[0].scatter(obs_fit, pred_fit, 1, c='grey')
    sns.kdeplot(x=obs_fit[::20], y=pred_fit[::20], cmap="RdYlBu_r", fill=True, thresh=0.002, alpha=0.8, ax=axs[0])
    x_fit = np.linspace(0.05, 0.95, 100)
    # axs[0].plot(x_fit, fit.intercept + fit.slope * x_fit, color='k', lw=1.2, label=f'y={fit.slope:.2f}x{fit.intercept:+.2f}')
    axs[0].plot([0.0,0.95],[0.0,0.95],color='r')
    axs[0].legend(loc='upper left', frameon=False, fontsize=7)
    axs[0].set_xlim([0.0,0.95])
    axs[0].set_ylim([0.0, 0.95])
    ...
    fig.tight_layout()
    figToPath = current_dir + '/4_Figures/Fig02a_model_drivers_vod'
    plt.savefig(figToPath, dpi=900)

    # Export data to CSV
    export_df = sorted_df[['name', 'shap_mean', 'std', 'color']]
    export_df.to_csv(current_dir + '/2_Output/fig_2a_driver_importance_vod.csv', index=False)
