...
def resilience_temporal_plot(ax, tac, band_years=23,offset=0,smooth=0):
    ...
    if offset == 1:
        ...
        mean_val_modis0[[0,-1]] = mean_val_modis[[0,-1]]*1.01
        mean_val_modis = mean_val_modis0

    if smooth == 1:
        series = pd.Series(mean_val_modis)
        mean_val_modis =  series.rolling(window=4, min_periods=1, center=True).mean().values
...
if __name__ == '__main__':
    ...
    trend = np.load(current_dir + '/2_Output/spatial_resilience/resilience_trend_brdf.npy')*100
    ...
    resilience_spatial_plot(ax1, ax2, trend, scale=23, k_size=int(3))

    trend2 = np.load(current_dir + '/2_Output/spatial_resilience/resilience_trend_vod.npy')*100
    ...
    resilience_temporal_plot(axs[0], tac, band_years=12,smooth=1)
