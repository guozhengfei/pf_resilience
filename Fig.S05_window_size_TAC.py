...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplots(3, 1, figsize=(4 * 0.8, 9 * 0.8))

    for i in range(3):
        ar1_5yr_modis = np.load(current_dir + '/2_Output/spatial_resilience/ar1_'+str(i+3)+'yr_kndvi_modis_sg_rolling.npy')
        ...
        axs[i].fill_between(time_label_modis, mean_val_modis - se, mean_val_modis + se, color='k', alpha=0.2, label='Standard Deviation')
        axs[i].set_ylim([0.43,0.49])

    fig.tight_layout()
    figToPath = current_dir + '/4_Figures/FigS05_window_size_TAC'
