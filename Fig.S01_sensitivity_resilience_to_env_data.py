...
def model_shap_obs(array):
    ...
    x2 = shap_values_ebm.data
    y2 = shap_values_ebm.values
    #slope = st.linregress(x2[~mask], y2[~mask]).slope
    return np.hstack([x2,y2]) # 0-10: obs; 11-21: shap_values
...
if __name__ == '__main__':
    ...
    plt.figure(); plt.plot(np.nanmean(alt_annual_data,axis=0))
    ...
    array = np.stack(
        [vpd_annual_data, srad_annual_data, pr_annual_data, tmean_annual_data, sT_annual_data, sm_annual_data,
         alt_annual_data,ndvi_annual_data, gsl_annual_data,lai_annual_data,tac_annual_data
         ], axis=2)  # remove phenology

    array = array[::50,:,:]
    divided_arrays = np.split(array, array.shape[0], axis=0)

    with mp.Pool(4) as pool:
        # results = list(pool.map(model_shap_obs, divided_arrays))
        results = list(tqdm(pool.imap(model_shap_obs, divided_arrays), total=len(divided_arrays), desc="Processing"))
    results_array = np.array(results)
    output_file = current_dir+'/2_Output/Temporal/Fig_S01_obs_shap.npy.npz'
    np.savez(output_file, array1=results_array, array2=mask)