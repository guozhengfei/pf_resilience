...
if __name__ == '__main__':
    ...
    ET1 = tf.imread(current_dir + '/1_Input/AET_annual_stack_1990_2024.tif')*0.1*12
    ...
    dataset = rasterio.open(current_dir + '/1_Input/AET_annual_stack_1990_2024.tif')
    ...
    # 在第一个子图（ax0）上绘图
    ax0.plot(np.linspace(1992, 2023, 32), evi_yr.values[2:,0], 'o-')
    coef = st.linregress(np.linspace(1992, 2023, 32), evi_yr.values[2:,0])
    ax0.plot([1992, 2023], [1992 * coef.slope + coef.intercept, 2023 * coef.slope + coef.intercept], 'r-')
    ...
    fig.tight_layout()
    figToPath = current_dir + '/4_Figures/R1_04_terra_ET'
    plt.savefig(figToPath, dpi=900)

    plt.figure(); plt.hist(trends[:,0])
