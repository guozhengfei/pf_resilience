...
if __name__ == '__main__':
    ...
    for yr in range(yr_num):
        start_index = (yr - ws) * bands_year
        end_index = (yr + ws) * bands_year
        if yr < ws: start_index = 0; end_index = 2*ws * bands_year
        if yr > (yr_num - ws): start_index = (yr_num - 2*ws) * bands_year; end_index = yr_num * bands_year
    ...
    # plt.figure(); plt.plot(np.linspace(2000,2023+21/23,551),np.nanmean(ar1_res_5sg,axis=0))

    Result = np.empty_like(EVI_yr[:,:-1])
    Result[~mask,:] = ar1_res_5sg
    output_file = current_dir + '/2_Output/spatial_resilience/ar1_5yr_kndvi_modis_sg_rolling_4.npy'
