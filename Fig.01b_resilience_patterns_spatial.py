...
if __name__ == '__main__':
    ...
    np.sum(trend_all[~np.isnan(trend_all)] > 0) / trend_all[~np.isnan(trend_all)].shape[0]

    fig, axs = plt.subplots(1,3,figsize=(4.5,1.5),sharey=True)
    axs[0].hist(trend_all[~np.isnan(trend_all)], 20, ec='k', range=[-0.02, 0.02])
    axs[1].hist(trend_07[~np.isnan(trend_07)], 20, ec='k', range=[-0.1, 0.1])
    axs[2].hist(trend_22[~np.isnan(trend_22)], 20, ec='k', range=[-0.04, 0.04])
    fig.tight_layout()
    figToPath = current_dir + '/4_Figures/Fig01_resilience_hist'
    ...
    this1 = ax1.pcolormesh(grid_longitudes, grid_latitudes, trend_all_map,
    ...
    this2 = ax2.pcolormesh(grid_longitudes, grid_latitudes, trend_07_map ,
    ...
    this3 = ax3.pcolormesh(grid_longitudes, grid_latitudes, trend_23_map,
                           cmap='RdBu_r', vmin=-0.015, vmax=0.015,
                           transform=ccrs.PlateCarree())
    ax3.coastlines()
    ax3.set_boundary(circle, transform=ax3.transAxes)
    plt.colorbar(this3, orientation='horizontal', label='ar1_trend_08-23', fraction=0.03, pad=0.05)
    fig.tight_layout()
    figToPath = current_dir + '/4_Figures/Fig01_resilience_patterns_spatialal'
    ...
    plt.bar(0,np.nanmean(trend_10),yerr=np.nanstd(trend_10)*0.05,color='C0',width=0.3,ec='k', hatch='//')
    plt.bar(0.3,np.nanmean(trend_23),yerr=np.nanstd(trend_23)*0.05,color='C0',width=0.3,ec='k')
    ...
    trend_10 = trend_07[index]
    trend_23 = trend_22[index]
    plt.bar(1,np.nanmean(trend_10),yerr=np.nanstd(trend_10)*0.05,color='C1',width=0.3,ec='k', hatch='//')
    plt.bar(1.3,np.nanmean(trend_23),yerr=np.nanstd(trend_23)*0.05,color='C1',width=0.3,ec='k')
    np.sum(trend_10>0)/np.sum(~np.isnan(trend_10))
    ...
    trend_10 = trend_07[index]
    trend_23 = trend_22[index]
    plt.bar(2,np.nanmean(trend_10),yerr=np.nanstd(trend_10)*0.05,color='C2',width=0.3,ec='k', hatch='//')
    plt.bar(2.3,np.nanmean(trend_23),yerr=np.nanstd(trend_23)*0.05,color='C2',width=0.3,ec='k')
    np.sum(trend_10>0)/np.sum(~np.isnan(trend_10))
    ...
    trend_10 = trend_07[index]
    trend_23 = trend_22[index]
    plt.bar(3,np.nanmean(trend_10),yerr=np.nanstd(trend_10)*0.05,color='C3',width=0.3,ec='k', hatch='//')
    plt.bar(3.3,np.nanmean(trend_23),yerr=np.nanstd(trend_23)*0.05,color='C3',width=0.3,ec='k')
    np.sum(trend_10>0)/np.sum(~np.isnan(trend_10))
    ...
    trend_10 = trend_07[index]
    trend_23 = trend_22[index]
    plt.bar(4,np.nanmean(trend_10),yerr=np.nanstd(trend_10)*0.05,color='C4',width=0.3,ec='k', hatch='//')
    plt.bar(4.3,np.nanmean(trend_23),yerr=np.nanstd(trend_23)*0.05,color='C4',width=0.3,ec='k')
    ...
    # #Wetland
    # index = (PFT==11)
    # trend_10 = ar1_trend_10[index]
    # trend_23 = ar1_trend_23[index]
    # ax3.bar(5,np.nanmean(trend_10),yerr=np.nanstd(trend_10)*0.05,color='gray',width=0.3,ec='k', hatch='//')
    # ax3.bar(5.3,np.nanmean(trend_23),yerr=np.nanstd(trend_23)*0.05,color='gray',width=0.3,ec='k')
    # np.sum(trend_10>0)/np.sum(~np.isnan(trend_10))

    fig.tight_layout()
    # figToPath = r'D:\Projects\Project_pf\Figures\Fig_1b_ar1_trend_kndvi_gimms'
    # plt.savefig(figToPath, dpi=900)
#
# fig, axs = plt.subplots(1,2,figsize=(4,2),sharey=True,sharex=True)
# mask = np.isnan(trends[:,0]) | (trends[:,1]>0.05) | np.isnan(trends[:,2]) | (trends[:,3]>0.05) | np.isnan(trends[:,4]) | (trends[:,5]>0.05)
# trend_val = trends[~mask,0]*23
# neg = trend_val[trend_val<0]
# pos = trend_val[trend_val>0]
# axs[0].hist(neg,bins=10,range=[-0.015, 0.015],color = 'gray', ec='black')
# axs[0].hist(pos,bins=10,range=[-0.015, 0.015],color = 'gray', ec='black')
#
# trend_val = trends[~mask,2]*23
# neg = trend_val[trend_val<0]
# pos = trend_val[trend_val>0]
# ax3.hist(neg,bins=10,range=[-0.015, 0.015],color = 'gray', ec='black')
# ax3.hist(pos,bins=10,range=[-0.015, 0.015],color = 'gray', ec='black')
# fig.tight_layout()
# figToPath = r'D:\Projects\Project_pf\Figures\Fig_ar1_trend_hist_kndvi_gimms'
# plt.savefig(figToPath, dpi=600)